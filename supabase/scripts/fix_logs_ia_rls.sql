-- Corrige/reaplica RLS dos logs de IA (seguro para rodar mais de uma vez)

alter table public.logs_ia enable row level security;

drop policy if exists "Usuarios veem logs de ia das avaliacoes da escola" on public.logs_ia;
drop policy if exists "Usuarios criam logs de ia das avaliacoes da escola" on public.logs_ia;
